...
from mpu_adafruit import MPU6050
import board
import rclpy
...
class IMUNode(Node):
    def __init__(self):
        # Node Init
        super(IMUNode, self).__init__(node_name="imu_node")

        # get ROS/Duckiebot parameters

        self.declare_parameter("veh", "")
        self.declare_parameter("connectors", [""])
        self.declare_parameter("polling_hz", 0.0)
        self.declare_parameter("temp_offset", 0.0)
        self.declare_parameter("ang_vel_offset", [0.0])
        self.declare_parameter("accel_offset", [0.0])

        self._veh = self.get_parameter('veh').get_parameter_value().string_value
        i2c_connectors = self.get_parameter("connectors").value
        self._i2c_connectors = [{elem.split(":")[0]: int(elem.split(":")[1]) for elem in conn.split(",")} for conn in i2c_connectors]
        self.get_logger().info(f"{self._i2c_connectors}")
        ...
        self._gyro_offset = self.get_parameter("ang_vel_offset").value
        self._accel_offset = self.get_parameter("accel_offset").value
    ...
    def zero_sensor(self, request, response):
        acc_data = self._sensor.acceleration
        gyro_data = self._sensor.gyro
        temp_data = self._sensor.temperature
